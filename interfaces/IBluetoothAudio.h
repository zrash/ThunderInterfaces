...
namespace Exchange {
    ...
    namespace IBluetoothAudio {
        ...
        struct EXTERNAL ISink : virtual public Core::IUnknown {
            ...
            struct EXTERNAL IControl : virtual public Core::IUnknown {
                ...
                // @brief Acquires the sink device and configures the streaming session
                ...
                // @retval ERROR_ILLEGAL_STATE The audio sink is not connected or already acquired
                // @retval ERROR_NOT_SUPPORTED Stream format or codec is not supported by the sink or otherwise invalid
                ...
                // @brief Releases the sink device and closes the streaming session
                // @retval ERROR_ILLEGAL_STATE The audio sink has not been acquired
                // @retval ERROR_ASYNC_FAILED Device error, failed to close the device
                ...
                // @param position Current playback position since connecting the sink (in miliseconds)
                // @retval ERROR_ILLEGAL_STATE The audio sink has not been acquired
        ...
        struct EXTERNAL ISource : virtual public Core::IUnknown {
            ...
            struct EXTERNAL IControl : virtual public Core::IUnknown {
                ...
                struct Format {
                    ...
                    uint8_t Resolution      /* @brief Sampling resolution in bits per sample (e.g. 16) */ ;
                    uint8_t Channels        /* @brief Number of audio channels in the source stream (e.g. 2) */ ;
                    // Sample data is always signed little endian. 
                };

                struct EXTERNAL ICallback : virtual public Core::IUnknown {

                    // @brief Notifies that the capture device was acquired 
                    // @param connector Name of the shared location the sample data will be written to
                    // @brief properties Audio properties of the incoming stream
                    // @retval ERROR_NOT_SUPPORTED Format of the incoming stream is not supported
                    // @retval ERROR_OPENING_FAILED Failed to open the shared location
                    virtual uint32_t Acquired(const string& connector, const Format& format) = 0;

                    // @brief Notifies that the capture device was released 
                    virtual void Relinquished() = 0;

                    // @brief Notifies that the source changed playback speed 
                    virtual void SpeedChanged(const int8_t speed) = 0;
                };

                // @brief Registers the control callback
                // @param Callback to register or nullptr to deregister
                // @retval ERROR_ILLEGAL_STATE A calback is already registered
                virtual uint32_t Callback(ICallback* callback) = 0;

                // @brief Forcefully releases the capture device and closes the streaming session
                // @retval ERROR_ILLEGAL_STATE The audio capture device has not been acquired
                // @retval ERROR_ASYNC_FAILED Device error, failed to release
                virtual uint32_t Relinquish() = 0;

                // @brief Gets current playback time
                // @param position Current playback position since the capture device was acquired (in miliseconds)
                // @retval ERROR_ILLEGAL_STATE The audio sink has not been acquired
                virtual uint32_t Time(uint32_t& position /* @out */) const = 0;
            };

            enum state : uint8_t {
                UNASSIGNED,             // No Bluetooth device assigned for audio capture
                DISCONNECTED,           // Capture device assigned but no source connected
                CONNECTED,              // Audio source connected, but not yet configured
                READY,                  // Capture device ready for receiving audio
                CAPTURING               // Audio is being recieved
            };
            ...
            struct ICallback : virtual public Core::IUnknown {
                ...
                // @brief Signals audio capture state change or stream properties update
                virtual void Updated() = 0;
            };

            // @brief Registers the state update callback
            // @param Callback to register or nullptr to deregister
            // @retval ERROR_ILLEGAL_STATE A calback is already registered with the audio source
            virtual uint32_t Callback(ICallback* callback) = 0;

            // @brief Assigns a Bluetooth device for audio capture
            // @param address Address of the bluetooth device to assign
            // @retval ERROR_ALREADY_CONNECTED A device is already assigned
            virtual uint32_t Assign(const string& address) = 0;

            // @brief Revokes a Bluetooth device from audio capture
            // @retval ERROR_ALREADY_RELEASED No device is currently assigned
            virtual uint32_t Revoke() = 0;

            // @property
            // @brief Current audio source state
            virtual uint32_t State(state& sourceState /* @out */) const = 0;

            // @property
            // @brief Audio source type
            // @retval ERROR_ILLEGAL_STATE The source device is not connected
            virtual uint32_t Type(devicetype& type /* @out */) const = 0;
            ...
            // @retval ERROR_ILLEGAL_STATE Currently not capturing
            virtual uint32_t Stream(StreamProperties& properties /* @out */) const = 0;
    ...
    using IBluetoothAudioSink = IBluetoothAudio::ISink;
    using IBluetoothAudioSource = IBluetoothAudio::ISource;
